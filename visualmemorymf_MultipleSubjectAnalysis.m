...
% Qualifying a subject's data as feasible:
% Eliminating any points above (Mean + 2*SD) and below (Mean - 2*SD).
 meanPerPerson_LocCompare = mean(subjectLocCompare,2);
 meanPerGroup_LocCompare = mean(meanPerPerson_LocCompare);
 meanPerPerson_ContCompare = mean(subjectContCompare,2);
 meanPerGroup_ContCompare = mean(meanPerPerson_ContCompare);
 stdContCompare = std(meanPerPerson_ContCompare); %95 percent of data should be within this range.
 stdLocCompare = std(meanPerPerson_LocCompare); %95 percent of data should be within this range.
 
 %checking for each person - is the data within this range? qualifying or
