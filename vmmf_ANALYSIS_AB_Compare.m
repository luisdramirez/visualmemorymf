...
experiments = {'Perception Condition A','Visual Working Memory A','Perception Condition B','Working Memory Consition B'};
...
for i = 1:length(visualmemory_subjectsRan)
      filename = strcat('data_visualmemorymf_exp_',visualmemory_subjectsRan{1,i},'.mat');
...
numContrasts = 5;
numExperiments = numel(experiments); %4 experiments, PA WMA PB WMB
numSubjects = round(size(visualmemory_subjectsRan,2)); 
...
%% EXPERIMENT LOOP %%
for e = 1:numel(experiments)/2
    
    %Figure Title
    if indvFiguresOn
        figure(e)
        if e  == 1
            set(gcf, 'Name', 'Perception (Location then Contrast)', 'Color', [1 1 1])
        elseif e == 2
            set(gcf, 'Name', 'Visual Working Memory (Location then Contrast)', 'Color', [1 1 1])
        elseif e == 3
            set(gcf, 'Name', 'Visual Working Memory (Contrast then Location)', 'Color', [1 1 1])
        elseif e == 4
            set(gcf, 'Name', 'Visual Working Memory (Contrast Then Location', 'Color', [1 1 1])
        end
    end
    ...
    %% SUBJECT LOOP %%
    % MODIFICATION: now loop through the A and B matrix for each order.
    subjCount = 0;
    for subject = 1:size(overallData.subjectDataA)
        %Specify order according to the subjectsran cell
        firstOrder = visualmemory_subjectsRan{2,subject};
        secondOrder = visualmemory_subjectsRan{3,subject};
        
        subjCount = subjCount + 1; % Subject Counter
        
        if e == 1 %Perception - A
            % Perception Experiment - A
            baselineMat(subjCount,:) = overallData.meanContABL(subjCount,:);
            variableMat(subjCount,:) = overallData.meanContAPerception(subjCount,:);
            
        elseif e == 2
            % Working Memory Experiment - A
            baselineMat(subjCount,:) = overallData.meanContBBL(subjCount,:);
            variableMat(subjCount,:) = overallData.meanContBWM(subjCount,:);
        end
    end
        ...
        indv_r2_coll = zeros(2,size(overallData.master_subjectData,1),2);
